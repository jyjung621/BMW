...
public class NYSCateDao {
	...
	public List<Category> list(int startRow, int endRow) throws SQLException {
		...
		String sql = "SELECT * FROM (SELECT ROWNUM rn, A.* FROM (SELECT * FROM category ORDER BY name ) A ) where rn between ? and ?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
	...
	public int delete(String[] val) throws SQLException {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int result = 0;
//		Category cate = new Category();
