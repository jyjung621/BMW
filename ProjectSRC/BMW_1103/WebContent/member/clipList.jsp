...
<div class="header">
		<h1><%=id %>님의 <a href='../myPage.do'>My Page</a></h1>
		<%-- <label for="file"><img class="photo3" src="BMW/${savefiles.get(0).getFilePath() }/${savefiles.get(0).getSavedFileName()}"/></label> --%>
		<c:if test="${count > 0 }">
			<label for="file"><img class="photo3" src="/BMW${savefile.getFilePath() }/${savefile.getSavedFileName()}"></label>
		</c:if>
	
	<form method="POST" action="memberPicPro.do" enctype="multipart/form-data">
	...
	<div class="topnav">
		<a href="mainForm.jsp">홈으로</a>
		<a href="clipList.do?view=1">나의 스크랩</a>
		<a href="#">내가 쓴 리뷰</a> 
		<a href="/BMW/member/CommentsList.do?memberId=<%=id%>">내가 쓴 댓글</a>
		<a href="/BMW/member/KJWupdateForm.do?memberId=<%=id%>">회원정보 수정</a>
