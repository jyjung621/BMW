		...
		<tr>
			<td colspan="7" ><br/>
				...
				<!-- <select name="name">
					...
					<option value="name">성분명</option>
					<option value="grade">위험도 등급</option>
				</select> -->
	...
	<c:choose>
		<c:when test="${keyWord != null}">
			<c:if test="${startPage > blockSize}">
				<a href="IngtList.do?pageNum=${startPage-blockSize}&keyWord=${keyWord}">[이전]</a>
			</c:if>	
			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<a href="IngtList.do?pageNum=${i}&keyWord=${keyWord}">[${i}]</a>
			</c:forEach>
			<c:if test="${endPage < pageCnt}">
				<a href="IngtList.do?pageNum=${startPage+blockSize}&keyWord=${keyWord}">[다음]</a>
			</c:if>
		</c:when>
		<c:otherwise>
			<c:if test="${startPage > blockSize}">
