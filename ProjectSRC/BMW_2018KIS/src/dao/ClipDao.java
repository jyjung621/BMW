...
public class ClipDao {
	...
	// Service -> DAO (6단계)
		public int getTotalCnt(String id) throws SQLException{
			int result = 0;
			Connection conn = null;
			String sql = "select count(*) from clip where memberid=?";
	...
	public List<Clip> list(String id, int startRow, int endRow) throws SQLException {
		...
		String sql = "select * from (select rownum rn, a.* from (select mainNo, subNo, title, regDate from clip where memberId=? order by regDate) a) "
...
//			private String regDate;
	...
	public int insert(String memberId, int boardNo, String title) throws SQLException {
		...
		int result = 0;
		ResultSet rs = null;
		String sql1 = "insert into clip values(?,?,?,?,fn_DateToChar(sysdate))";
		try {
