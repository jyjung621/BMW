						...
						<section style="width: 85%;">
							<div>
								...
								<c:if test="${view == 1 }">
									<h2>스크랩한 리뷰</h2><br>
									<tr>
										<td><input type="button" value="게시글 보기"
											onclick="location.replace('clipList.do?view=2')"></td>
										<c:if test="${totCnt > 0 }">
											<td><input type="button" value="편집"
												onclick="location.replace('clipList.do?pageNum=${currentPage}&view=1&gubun=1')"></td>
										</c:if>
											...
											<c:forEach var="clip" items="${list }">
												<tr>
													...
													<td class="left" width=200><a
														href='content.do?num=${clip.mainNo}&pageNum=${currentPage}'>${clip.title}</a>
								...
								<c:if test="${view == 2 }">
									<h2>스크랩한 게시글</h2><br>
									<tr>
										<td><input type="button" value="리뷰 보기"
											onclick="location.replace('clipList.do?view=1')"></td>
										<c:if test="${totCnt > 0 }">
											<td><input type="button" value="편집"
												onclick="location.replace('clipList.do?pageNum=${currentPage}&view=2&gubun=2')"></td>
										</c:if>
									</tr>
									<table>
										<tr>
										...
										<c:if test="${totCnt > 0 }">
											<c:forEach var="clip" items="${list }">
												<tr>
													<td>${clip.mainNo }</td>
													<td class="left" width=200><a
														href='/BMW/board/boardContent.do?boardNo=${clip.mainNo }'>${clip.title}</a>
