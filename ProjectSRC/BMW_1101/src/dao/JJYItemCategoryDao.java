...
public class JJYItemCategoryDao {
	...
	// 검색시에 구하는 totCnt
	public int getSearchTotalCnt(int categoryNo, String keyword) throws SQLException {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		int result = 0;
	...
	// 검색결과 내 재검색 메소드
	public List<ItemInfo> itemCateList2(int categoryNo, int sortName, int startRow, int endRow, String keyword)
			throws SQLException {
		...
		ResultSet rs = null;
		ItemInfo item = null;
		String sql0 = "select * from (select rownum rn, a.*"
				+ "from (select * from iteminfo where name like ? order by rating) a" + ") where rn between ? and ?";
		String sql1 = "select * from (select rownum rn, a.*"
				+ "from (select * from iteminfo where categoryno=? and name like ? order by name) a"
				+ ") where rn between ? and ?";
		String sql2 = "select * from (select rownum rn, a.*"
				+ "from (select * from iteminfo where categoryno=? and name like ? order by rating desc) a"
				+ ") where rn between ? and ?";
		String sql3 = "select * from (select rownum rn, a.*"
				+ "from (select * from iteminfo where categoryno=? and name like ? order by likecnt desc) a"
				+ ") where rn between ? and ?";
		String sql4 = "select * from (select rownum rn, a.*"
				+ "from (select * from iteminfo where categoryno=? and name like ? order by regdate desc) a"
				+ ") where rn between ? and ?";
	...
	public List<ItemInfo> itemCateList(int categoryNo, int sortName, int startRow, int endRow) throws SQLException {
		...
		ResultSet rs = null;
		ItemInfo item = null;
		String sql0 = "select * from (select rownum rn, a.*" + "from (select * from iteminfo order by ";
		String sql01 = ") a) where rn between ? and ?";
		String sql1 = "select * from (select rownum rn, a.*"
				+ "from (select * from iteminfo where categoryno=? order by name) a" + ") where rn between ? and ?";
		String sql2 = "select * from (select rownum rn, a.*"
				+ "from (select * from iteminfo where categoryno=? order by rating desc) a"
				+ ") where rn between ? and ?";
		String sql3 = "select * from (select rownum rn, a.*"
				+ "from (select * from iteminfo where categoryno=? order by likecnt desc) a"
				+ ") where rn between ? and ?";
		String sql4 = "select * from (select rownum rn, a.*"
				+ "from (select * from iteminfo where categoryno=? order by regdate desc) a"
				+ ") where rn between ? and ?";
		try {
			conn = getConnection();
			if (categoryNo == 0) {
