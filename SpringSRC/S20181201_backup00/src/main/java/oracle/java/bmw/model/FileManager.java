...
public class FileManager {
	...
	// 파일 업로드를 위한 생성자
	public FileManager(String filePath, String formName, MultipartHttpServletRequest multipartRequest) {
	...
	// ***** Getter & Setter ******//
	// Real 파일업로드 Setter
	public FileManager setUpload(String filePath, String formName, MultipartHttpServletRequest multipartRequest) {
		this.formName = formName;
		this.filePath = filePath;
		this.multipartRequest = multipartRequest;
		fileNames = new HashMap<String, List<String>>();
...
//		Map<String, List<String>> fileNames = new HashMap<String, List<String>>();
		...
		for(int i=0; i<mf.size(); i++) {
			System.out.println("==================== "+ mf.size() +"중 반복실행 " + i + " ==================================");
...
//				String fileFullPath = savedFileName;
				...
				try {
					...
					// 파일 저장
					File fileFull = new File(filePath, savedFileName);
//					mf.get(i).transferTo(targetDir);
					...
					saveFiles.setFilePath(filePath);
